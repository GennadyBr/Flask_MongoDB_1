...
prefix = '/flask_mongo_app'  # nginx prefix name and container name
app = Flask(__name__)
...
def show_user():
    ...
    return render_template("users_list.html", users=users, prefix=prefix)


@app.route(f"{prefix}/add", methods=['GET', 'POST'])
def add_user():
    """Добавление новой записи в базу данных"""
    if request.method == 'GET':
        return render_template("user_details.html", user={}, prefix=prefix)
...
@app.route(f"{prefix}/update/<int:id>", methods=['GET', 'POST'])
def update_user(id):
    ...
    if request.method == 'GET':
        for user in collection.find({'ID': id}):
            ...
            return render_template("user_details.html", user=user_list[0], prefix=prefix)
    if request.method == 'POST':
        name = request.form["name"]
        year = int(request.form["year"])
        salary = float(request.form["salary"])
...
@app.route(f"{prefix}/delete/<int:id>")
def delete_user(id):
